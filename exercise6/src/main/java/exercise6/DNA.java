...
//        }
        ...
        int snj = seqNoJunk.length();
        int re = restrictionEnzyme.length();
        int j = 0;
        
        for(int i = 0; i <= snj - re; ) {
        	String str = seqNoJunk.substring(i, i + re);
        	if(!str.equals(restrictionEnzyme)){
        		newSeqBuilder.append(seqNoJunk.charAt(i));
        		i ++;
        	}
        	else {
        		newSeqBuilder.append(seqNoJunk.substring(i, i + splicePosition));
        ...
        newSeqBuilder.append(seqNoJunk.substring(j));
	...
	public Set<String> getCodons() {
		return codons;
	}

	public void setCodons(Set<String> codons) {
		this.codons = codons;
	}
